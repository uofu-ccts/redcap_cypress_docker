...
git pull
git fetch https://github.com/uofu-ccts/redcap_cypress_docker utah-v16.0.11
commitsBehindMain=`git log --oneline ..FETCH_HEAD | wc -l`
if [ $commitsBehindMain != 0 ]; then
    echo
    echo Please either checkout the utah-v16.0.11 branch for redcap_cypress_docker, or merge it into your working branch.
    exit
fi
...
git fetch https://github.com/uofu-ccts/redcap_rsvc utah-v16.0.11
rsvcBranchName=`git rev-parse --abbrev-ref HEAD`
if [ "$rsvcBranchName" = "staging" ] || [ "$rsvcBranchName" = "main" ] || [ "$rsvcBranchName" = "utah-v16.0.11" ]; then
    ...
    if [ $commitsBehindStaging != 0 ]; then
        set +x
        echo
        echo Please merge the latest from the 'utah-v16.0.11' branch into your redcap_rsvc branch.
...
cd redcap_cypress
git pull
git fetch https://github.com/uofu-ccts/redcap_cypress utah-v16.0.11
commitsBehindMaster=`git log --oneline ..FETCH_HEAD | wc -l`
if [ $commitsBehindMaster != 0 ]; then
    echo
    echo Please either checkout the utah-v16.0.11 branch for redcap_cypress, or merge it into your working branch.
    exit
fi
...
cd redcap_docker
git pull
git fetch https://github.com/uofu-ccts/redcap_docker utah-v16.0.11
commitsBehindMain=`git log --oneline ..FETCH_HEAD | wc -l`
if [ $commitsBehindMain != 0 ]; then
    echo
    echo Please either checkout the utah-v16.0.11 branch for redcap_docker, or merge it into your working branch.
    exit
fi
